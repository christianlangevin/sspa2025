...
fname = data_path / "k_aq_SI.tif"
kaq = flopy.utils.Raster.load(fname)

fname = data_path / "k_clay_SI.tif"
kclay = flopy.utils.Raster.load(fname)

fname = data_path / "top_SI.tif"
top_base = flopy.utils.Raster.load(fname)

fname = data_path / "bottom_SI.tif"
bot = flopy.utils.Raster.load(fname)

fname = data_path / "lake_location_SI.tif"
...
def get_mf6gwf_sim(lake_concentration=1.0, recharge_concentration=0.0):
    ...
    # reset lake concentration to value passed in
    lno, lstage, gwfno, lakc = lakpak_data[0]
    lpd = [(lno, lstage, gwfno, lake_concentration)]
...
def get_mf6gwt_sim(initial_concentration=0.0):
    ...
    # observations
    obsdata = []
    for lay, icpl, rate, wname in welspd:
        t = (f"{wname}", "concentration", (lay, icpl))
        obsdata.append(t)
    obs_recarray = {f"{name}.obs.csv": obsdata}
...
def plot_head_results(sim_mf6gwf):
...
def plot_conc_results(sim_mf6gwt):
